...
from rde.utils.mixed_dataloader import MixedDatasetManager, per_complex_corr
from rde.models.dg_network_str import NetworkStr# 结构模型
from rde.models.dg_network_seq import NetworkSeq# 序列模型
from rde.models.dg_network_seq_finetune import NetworkSeqAlign
Network = {'structure':NetworkStr, 'seq':NetworkSeq}
...
#     parser.add_argument('--num_cvfolds', type=int, default=5)
#     parser.add_argument('--logdir', type=str, default='./log_mixed_skempi_sabdab_pdbbind')
    ...
    struc_ckpt = torch.load(args.pretrained_model, map_location=args.device)
    
    model_factory=NetworkSeqAlign

    cv_mgr = CrossAlign(
        model_factory=model_factory,
        config=config, 
        struc_config=struc_ckpt['config']['model'],
        num_cvfolds=num_cvfolds
    ).to(args.device)
    it_first = 1

    cv_mgr.load_struc_model_state_dict(struc_ckpt['model'], )
    ...
    def train(it):
        ...
        loss = sum_weighted_losses_v0(loss_dict, config.train.loss_weights)
...
#         try:
        optimizer.zero_grad()
        loss.backward()
        orig_grad_norm = clip_grad_norm_(model.parameters(), config.train.max_grad_norm)# 裁剪梯度
        optimizer.step()
...
#                     loss_dict, output_dict = model(batch)
                    ...
                    if 'receptor' in batch:
                        batchsize = batch['receptor']['size'] 
                        pdbcodes = batch['receptor']['pdbcode']
                        mutstrs = batch['receptor']['mutstr']
                    else:
                        batchsize = batch['size'] 
                        pdbcodes = batch['pdbcode']
                        mutstrs = batch['mutstr']
                    scalar_accum.add(name='loss', value=loss, batchsize=batchsize, mode='mean')
                    for complex, mutstr, dg_true, dg_pred in zip(pdbcodes, mutstrs, output_dict['dG_true'], output_dict['dG_pred']):
                        results.append({
                            'complex': complex,
                            'mutstr': mutstr,
