...
def cross_pairwise_dihedrals(pos_atoms_q, pos_atoms_kv):
    """
    Args:
        pos_atoms_q:  (N, L_q, A, 3).
        pos_atoms_kv:  (N, L_kv, A, 3).
    Returns:
        Inter-residue Phi and Psi angles, (N, L_q, L_kv, 2).
    """
    N, L_q = pos_atoms_q.shape[:2]
    N, L_kv = pos_atoms_kv.shape[:2]
    
    pos_N_q  = pos_atoms_q[:, :, BBHeavyAtom.N]   # (N, L_q, 3)
    pos_CA_q = pos_atoms_q[:, :, BBHeavyAtom.CA]
    pos_C_q  = pos_atoms_q[:, :, BBHeavyAtom.C]
    
    pos_N_kv  = pos_atoms_kv[:, :, BBHeavyAtom.N]   # (N, L_kv, 3)
    pos_CA_kv = pos_atoms_kv[:, :, BBHeavyAtom.CA]
    pos_C_kv  = pos_atoms_kv[:, :, BBHeavyAtom.C]

    ir_phi = dihedral_from_four_points(
        pos_C_q[:,:,None].expand(N, L_q, L_kv, 3), 
        pos_N_kv[:,None,:].expand(N, L_q, L_kv, 3), 
        pos_CA_kv[:,None,:].expand(N, L_q, L_kv, 3), 
        pos_C_kv[:,None,:].expand(N, L_q, L_kv, 3)
    )
    ir_psi = dihedral_from_four_points(
        pos_N_q[:,:,None].expand(N, L_q, L_kv, 3), 
        pos_CA_q[:,:,None].expand(N, L_q, L_kv, 3), 
        pos_C_q[:,:,None].expand(N, L_q, L_kv, 3), 
        pos_N_kv[:,None,:].expand(N, L_q, L_kv, 3)
    )
    ir_dihed = torch.stack([ir_phi, ir_psi], dim=-1)
