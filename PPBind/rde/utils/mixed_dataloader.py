...
from rde.utils.data import PaddingCollate_mix, PaddingCollate_struc, PaddingCollate_mix_v2, PaddingCollate_seq, PaddingCollate_struc_infer
from rde.utils.transforms import get_transform
from rde.datasets import MixedDataset
import itertools


def per_complex_corr(df, pred_attr='dG_pred', limit=10):
    corr_table = []
    for cplx in df['complex'].unique():
        ...
        corr_table.append({
            'complex': cplx,
            'pearson': df_cplx[['dG', pred_attr]].corr('pearson').iloc[0,1],
            'spearman': df_cplx[['dG', pred_attr]].corr('spearman').iloc[0,1],
        })
...
class MixedDatasetManager(object):

    def __init__(self, config, num_cvfolds, num_workers=4, logger=BlackHole(), debug=False):
        super().__init__()
        self.config = config
        ...
        for fold in range(num_cvfolds):
            train_iterator, val_loader, train_cplx, val_cplx = self.init_loaders(fold)
            self.train_split.append(train_cplx)
            self.val_split.append(val_cplx)
    ...
    def init_loaders(self, fold):
        config = self.config
        mixed_dataset_ = functools.partial(
            MixedDataset,
        ...
        val_dataset = mixed_dataset_(split='val', transform=get_transform(config.data.transform.val))

        train_cplx = set([e['pdbcode'] for e in train_dataset.entries])
        val_cplx = set([e['pdbcode'] for e in val_dataset.entries])
        
        if self.config.data.strict:
        ...
        if config.data.finetune:
            PaddingCollate=PaddingCollate_mix_v2()
        elif config.data.collate == 'mix':
            PaddingCollate=PaddingCollate_mix()
        elif config.data.collate == 'str':
            PaddingCollate= PaddingCollate_struc_infer() #PaddingCollate_struc()
        elif config.data.collate == 'seq':
            PaddingCollate=PaddingCollate_mix_v2()
        ...
        return train_iterator, val_loader, train_cplx, val_cplx
...
def overall_correlations(df):
    pearson = df[['dG', 'dG_pred']].corr('pearson').iloc[0,1]
    spearman = df[['dG', 'dG_pred']].corr('spearman').iloc[0,1]
    return {
...
def percomplex_correlations(df, return_details=False):
    ...
    for cplx in df['complex'].unique():
        ...
        corr_table.append({
            'complex': cplx,
            'pearson': df_cplx[['dG', 'dG_pred']].corr('pearson').iloc[0,1],
            'spearman': df_cplx[['dG', 'dG_pred']].corr('spearman').iloc[0,1],
        })
    corr_table = pd.DataFrame(corr_table)
...
def eval_skempi(df_items, mode, dG_cutoff=None):
    ...
    if dG_cutoff is not None:
        df_items = df_items.query(f"dG >= {-dG_cutoff} and dG <= {dG_cutoff}")
...
def eval_skempi_three_modes(results, dG_cutoff=None):
    df_all = eval_skempi(results, mode='all', dG_cutoff=dG_cutoff)
    df_single = eval_skempi(results, mode='single', dG_cutoff=dG_cutoff)
    df_multiple = eval_skempi(results, mode='multiple', dG_cutoff=dG_cutoff)
