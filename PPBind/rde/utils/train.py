...
def get_optimizer_finetune(cfg, model):
#     for name, param in model.named_parameters():
#         if "struc_model" in name:
...
def sum_weighted_losses(losses, weights):
    """
    Args:
        losses:     Dict of dict of tensors.
...
#     import pdb;pdb.set_trace()
    ...
    for k, v in losses.items():
        if v is None:
            continue
        loss_dict[k] = (v['value']*v['mask']).sum() / v['mask'].sum()
        if weights is None:
            loss = loss + loss_dict[k]
        else:
            loss = loss + weights[k] * loss_dict[k]
    return loss, loss_dict


def sum_weighted_losses_v0(losses, weights):
    """
    Args:
...
class CrossAlign(object):
    # only used to finetune on sequence

    def __init__(self, model_factory, config, struc_config, num_cvfolds):
        super().__init__()
        self.num_cvfolds = num_cvfolds
        self.models = [
            model_factory(config.model, struc_config)
            for _ in range(num_cvfolds)
        ]
        ...
        self.schedulers = []
        for model in self.models:
            optimizer = get_optimizer_finetune(config.train.optimizer, model)
            self.optimizers.append(optimizer)
            self.schedulers.append(get_scheduler(config.train.scheduler, optimizer))
    ...
    def load_struc_model_state_dict(self, state_dict):
        for sd, obj in zip(state_dict['models'], self.models):
            obj.struc_model.load_state_dict(sd)
